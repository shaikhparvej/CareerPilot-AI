...
export function EnhancedGrammarCheckClient() {
  ...
  // Use the robust speech recognition hook
  const {
    isListening,
    transcript,
    interimTranscript,
  ...
  const handleSpeechAnalysis = async (speechText: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Analysis ready with ${response.suggestions?.length || 0} suggestions.`,
        variant: "default",
      });
    } catch (error) {
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <CardTitle className="text-2xl font-headline flex items-center">
            <Mic className="mr-2 h-6 w-6 text-primary" /> Enhanced Extempore Practice
          </CardTitle>
          <CardDescription>
            {currentTopic ? (
              <>
                ...
                the microphone button to start or stop speaking.
              </>
            ) : (
        ...
        {currentTopic && speechApiSupported && (
          <CardFooter className="flex-col items-start space-y-4">
            ...
            <div className="w-full p-4 border rounded-md min-h-[120px] bg-muted/30">
              ...
              {!isListening && !transcript && !analysisResult && (
                <p className="text-sm text-muted-foreground italic">
                  Click &quot;Start Speaking&quot; to begin recording your extempore speech.
                </p>
              )}
            </div>
          </CardFooter>
...
export default EnhancedGrammarCheckClient;
